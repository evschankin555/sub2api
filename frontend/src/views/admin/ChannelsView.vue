...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel }, `No group selected for ${platformLabel}. Select at least one group or disable this platform.`))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel }, `${platformLabel} has a pricing entry without models. Add a model or remove the entry.`))
        activeTab.value = section.platform
        return
      }
    }
