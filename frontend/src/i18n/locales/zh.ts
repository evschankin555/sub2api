export default {
  ...
  admin: {
    ...
    groups: {
      ...
      copyAccounts: {
        accountCountLabel: '{name} ({count} 个账号)',
        title: '从分组复制账号',
      ...
      claudeMaxSimulation: {
        ...
        tooltip:
          '启用后，对没有上游 cache-write 用量的 Claude 模型，系统会确定性地将 tokens 映射为少量输入加 1h 缓存创建，同时保持总 token 数不变。',
        enabled: '已启用（模拟 1h 缓存）',
        disabled: '已禁用',
        hint: '仅调整用量计费日志中的 token 分类，不会持久化单请求映射状态。'
      },
      supportedScopes: {
        title: '支持的模型系列',
