  ...
  if (mode === 'whitelist') {
    ...
    for (const m of modelMappings) {
      ...
      if (!isValidWildcardPattern(from)) {
        console.warn(`[buildModelMappingObject] Invalid wildcard format, skipped: ${from}`)
        continue
      }
      // to 不允许包含通配符
      if (to.includes('*')) {
        console.warn(`[buildModelMappingObject] Target model cannot contain wildcard, skipped: ${from} -> ${to}`)
        continue
      }
      mapping[from] = to
    }
